...
**Unit dimensions** are basic physical quantities that can be measured. Figure \@ref(fig:tab1) lists the fundamental unit dimensions of some primary quantities along with the notation we use to denote them, and their corresponding [SI units](https://en.wikipedia.org/wiki/International_System_of_Units). 

```{r tab1, echo=FALSE, fig.cap="Fundamental Unit Dimensions of Primary Quantities.", fig.height=1}
knitr::include_graphics("./Figures/si_units.png")
```

Of course, SI units are not the only choice of units. The point is that mass, length, etc. are the quantities being measured. Furthermore, mass, length, etc. are fundamental in the sense that they can not be viewed as being made up of more basic quantities. On the other hand, the quantities listed in Figure \@ref(fig:tab2) are said to be **derived** because they can be viewed as being made up of more basic quantities. For example, velocity has derived dimensions of $\frac{\text{length}}{{\text{time}}}$. In order to simplify the language a bit, we will often say the dimensions of velocity are $\frac{\text{length}}{{\text{time}}}$, read "length per time." 

```{r tab2, echo=FALSE, fig.cap="Examples of Derived Units.", fig.width=2}
knitr::include_graphics("./Figures/physical_quantities.png")
```
...
Observe that a pure numbers (one that does not correspond to a physical measurement) like $6$, $0.12$, or $\pi$ are unitless. This is because pure numbers are fixed and do not depend on a choice of units. We will say that a pure number is **nondimensional**.   
...
4) Quantities on both sides of an equality (or inequality for that matter) must have the same unit dimensions. You can not compare or equate quantities with different units. 
...
**Exercise:** Suppose that we have a function $y(t)$, where $[y]=L$ and $[t]=T$. What are the unit dimensions for $\frac{d^2y}{dt^2}$? In general, if $y=f(x)$ is a function, what would be $[f''(x)]$?
...
are mathematically equivalent up to a change of units. The benefit of non-dimensionalizing is that $\xi = \gamma \tau^2 + \tau + 1$ does not depend on any particular choice of units and we have identified that the significance of the parameters is encapsulated in the ration $\frac{ac}{b^2}$. (Think about why this is true, a hint is to refer to the quadratic formula.) 
...
and the units cancel. This completes step 1) of the non-dimensionalization process. To complete step 2) we need to find a differential equation that is satisfied by $\xi(\tau)$. Let's proceed by computing the derivative $\frac{d\xi}{d\tau}$ of $\xi$ with respect to $\tau$. As you will see, the trick is to use the [chain rule](https://en.wikipedia.org/wiki/Chain_rule).
...
We will prove points 1) and 2) later after we have discussed phase-plane methods. Notice that dimensional analysis and non-dimensionalization have lead is to the conclusion that it is the ratio $\frac{\beta N}{\gamma}$ that is important. 

## Generic Rescaling

How do you choose an appropriate rescaling for the non-dimensionalization process? The answer is that it depends on the situation. For example, in a model there may be some parameters that have well-established values say from experiment or the literature. In such a case, you may want to use known parameter values to set the rescaling. In other cases, you might want to set a particular time-scale in which case that would play a role in your choice of rescaling. Another approach is to start with a generic rescaling and then proceed with the non-dimensionalization with the specific relationship between the model variables and parameters to be determined after non-dimensionalizing. Let's look at some examples of this idea. 
...
Notice that $\xi$ and $\tau$ are dimensionless because we have defined $\tilde{L}$ and $\tilde{T}$ to gave the same unit dimesions as $y$ and $t$, respectively. Next, substitute $y=\tilde{L}\xi$ and $t=\tilde{T}\tau$ into $y=at^2+bt+c$. We get
...
An excellent exercise is to attempt to reproduce the nondimensionalization found in research articles. As a first try, see if you can follow or work out on your own the nondimensionalization from one of the following papers: [@donofrioGeneralFrameworkModeling2005; @edelstein-keshetSimpleModelsTrailfollowing1994; @heraldGeneralModelInflammation2010]. 
